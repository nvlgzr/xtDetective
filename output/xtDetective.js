function start() {
    ...
    const originallyEnabled = current
        ? current.filter(isEnabled).map(extensionName)
        : [];
    const originallyDisabled = current
...
function pass(model) {
    ...
    if (noMoreToTest) {
        ...
        const { originallyEnabled, originallyDisabled, untested, toTest, passed, underSuspicion } = model;
...
function readTheDOM() {
    const dom = document?.querySelector("extensions-manager")?.shadowRoot?.querySelector("extensions-item-list")?.shadowRoot?.querySelectorAll("extensions-item");
...
function disableAll(names) {
    for (let name of names) {
        disable(name);
    }
}
function enableAll(names) {
    for (let name of names) {
        enable(name);
    }
}
function enable(name) {
    ...
    for (let node of currentlyDisabled()) {
        if (extensionName(node) === name && !isEnabled(node)) {
            ...
            let toggle = node.shadowRoot.querySelector("cr-toggle");
            toggle.click();
        }
        else {
...
function disable(name) {
    for (let node of readTheDOM()) {
        if (extensionName(node) === name && isEnabled(node)) {
            ...
            const toggle = node.shadowRoot.querySelector("cr-toggle");
            toggle.click();
        }
        else {
